...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( get_option('woocommerce_enable_review_rating') == 'yes' ) {
		...
		if ( $count > 0 ) {
			...
			echo '<h2 class="review-title">'.sprintf( _n('%s review for %s', '%s reviews for %s', $count, 'woocommerce'), '<span itemprop="ratingCount" class="count">'.$count.'</span>', wptexturize($post->post_title) ).'</h2>';

			echo '</div>';

		} else {

			echo '<h2 class="review-title">'.__( 'Reviews', 'woocommerce' ).'</h2>';
	...
	echo '</section><div id="review_form_wrapper"><div id="review_form">';
